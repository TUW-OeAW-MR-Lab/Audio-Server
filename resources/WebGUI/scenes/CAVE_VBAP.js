...
export function init()
{
  ...
  document.getElementById("CAVE_VBAP-status").innerText = "initialized";
  ...
  document.getElementById('CAVE_VBAP-Total_VU').addEventListener('updated', (e) => 
  {
    const vu = document.getElementById('CAVE_VBAP-Total_VU');
    document.getElementById('CAVE_VBAP-Total_VU-bar').style.height = String(Math.round(vu.innerText)+100) + '%';
    document.getElementById('CAVE_VBAP-Total_VU-number').innerText = Math.round(vu.innerText) + ' dB';
  });			

  document.getElementById('CAVE_VBAP-Total_Volume').addEventListener('updated', (e) => 
  {
    const vol = document.getElementById('CAVE_VBAP-Total_Volume');
  ...
  const subLabel = document.getElementById('CAVE_VBAP-Subwoofer_Volume') || document.getElementById('CAVE_VBAP-Total_Subwoofer');
  if (subLabel) {
    subLabel.addEventListener('updated', (e) => 
    {
      document.getElementById('subwoofer-slider').value = Math.round(subLabel.innerText);
...
{
  document.getElementById("btn-CAVE_VBAP-launch").disabled = true;
  document.getElementById("CAVE_VBAP-status").innerText = "initialized";
	document.getElementById("CAVE_VBAP-status").style.backgroundColor = "";	
	document.getElementById("CAVE_VBAP-Control_Version").innerText = "---";
	document.getElementById("CAVE_VBAP-Control_SampleRate").innerText = "---"; 
...
{
	state = document.getElementById("CAVE_VBAP-status");
	state.innerText = "connecting...";
	state.style.backgroundColor = "blue";
	sendResponse('/app/CAVE_VBAP/osc/Control/Response', 9336);
	sendNoArgs('/app/CAVE_VBAP/osc/Control/Version');
	secWaited = 0;
...
export function checkConnection()
{
	...
	secWaited += 100;
	state = document.getElementById("CAVE_VBAP-Control_Version");
	...
	switch (state.innerText)
	{
		case "---":
			...
			break;
		default: // done!
      console.log("CAVE_VBAP: connected");
      document.getElementById("btn-CAVE_VBAP-launch").disabled = false;
			state = document.getElementById("CAVE_VBAP-status");
...
export function SetDanteLedPcChannel3()
{
  ...
  {
    sendValue('/app/CAVE_VBAP/osc/VirtualSource/3/Switch', 1);
    sendValue('/app/CAVE_VBAP/osc/VirtualSource/3/Volume/Set', 0);
    sendValues3('/app/CAVE_VBAP/osc/VirtualSource/3/Position/Set', 0, 0, 3.2);
  }
  else
  { 
...
{
  setInputButtonExclusively('DANTE_CurvedLEDPC', 'CAVE_VBAP');
...
{
  setInputButtonExclusively('DANTE_Mobile', 'CAVE_VBAP');
  sendValue('/app/CAVE_VBAP/osc/VirtualSource/3/Switch', 0);
...
{
  quitApp('CAVE_VBAP', 'CAVE_VBAP', SceneModule.init);
  sendValue('/matrix/state/settings/sum_bus_master/*/mute', 1); // Mute all Sum Busses
}
